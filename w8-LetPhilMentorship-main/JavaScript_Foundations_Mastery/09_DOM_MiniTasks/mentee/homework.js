...
moviesTitleEl.textContent = "Movies List";
...
const movieCount = moviesListEl.querySelectorAll("li").length;
...
newMovieItem.textContent = "Matrix";
newMovieItem.classList.add("movie-item");
moviesListEl.appendChild(newMovieItem);
// STEP 8: Select one of the existing movie items (for example,
//         the first one) and remove it using .remove().
const firstMovieItem = moviesListEl.querySelector("li");
firstMovieItem.remove();
// STEP 9: (Optional) After adding/removing, update moviesInfo again
//         to show the new total number of movies.
moviesInfoEl.textContent = `You currently have ${moviesListEl.querySelectorAll("li").length} favorite movies in your list.`