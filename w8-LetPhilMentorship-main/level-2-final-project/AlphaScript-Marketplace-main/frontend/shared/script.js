...
// === SHARED HEADER + FOOTER + TICKER + WALLET + LANGUAGE ===
function renderNavbar() {
  const html = `
    <div class="logo"><a href="../index.html" style="color:#fff;text-decoration:none;">AlphaScript</a></div>
...
function renderFooter() {
  const html = `<footer>© 2026 AlphaScript. All rights reserved.</footer>`;
  document.body.insertAdjacentHTML('beforeend', html);
}

function renderTicker() {
  const html = `<div class="crypto-ticker"><div id="ticker-content" class="ticker-content"></div></div>`;
  document.body.insertAdjacentHTML('beforeend', html);
}
